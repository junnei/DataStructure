	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			...
			else if (itor.m_pCurPointer->next == m_pLast)
			{
				...
				pItem->next = m_pLast;
				itor.m_pCurPointer = pItem; // 아이템을 삽입.
				m_nLength++;
				return 1;
			}
			else
...
int DoublySortedLinkedList<T>::AddByBytes(T item)
{
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (itor.GetCurrentNode()->CompareByBytes(item) == LESS) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
				pItem->prev = itor.m_pCurPointer->prev;
...
int DoublySortedLinkedList<T>::AddByDate(T item)
{
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (itor.GetCurrentNode().data.CompareByDate(item) == LESS) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
				pItem->prev = itor.m_pCurPointer->prev;
...
int DoublySortedLinkedList<T>::AddByCount(T item)
{
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (itor.GetCurrentNode().data.CompareByCount(item) == GREATER) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
				pItem->prev = itor.m_pCurPointer->prev;
...
int DoublySortedLinkedList<T>::AddByDateR(T item)
{
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (itor.GetCurrentNode().data.CompareByDate(item) == GREATER) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
				pItem->prev = itor.m_pCurPointer->prev;
...
int DoublySortedLinkedList<T>::AddByCountR(T item)
{
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (item.CompareByCount(itor.GetCurrentNode().data) == LESS) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
				pItem->prev = itor.m_pCurPointer->prev;
